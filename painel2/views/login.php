...
<head>
  ...
  <title>Secatre Ambiental - Painel Cursos-login</title>
...
<body translate="no" >
  ...
  <form method="POST">
	...
	<div class="inputGroup inputGroup2">
		...
		<input type="password" name="senha" id="loginPassword" />
		<label id="showPasswordToggle" for="showPasswordCheck">Ver
...
function uncoverEyes() {
	...
	TweenMax.to(armR, 1.35, { rotation: -105, ease: Quad.easeOut, delay: .1, onComplete: function onComplete() {
...
function startBlinking(delay) {
	...
	blinking = TweenMax.to([eyeL, eyeR], .1, { delay: delay, scaleY: 0, yoyo: true, repeat: 1, transformOrigin: "center center", onComplete: function onComplete() {
			startBlinking(12);
		} });
}
...
function getPosition(el) {
	...
	while (el) {if (window.CP.shouldStopExecution(0)) break;
		if (el.tagName == "BODY") {
