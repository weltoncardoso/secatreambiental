...
function get_info_from_email($email) {
    ...
    $sql = "SELECT
            ...
            aluno_curso.date_cadastro as data,
            aluno_curso.file,
            alunos.nome as name,
            alunos.email,
            ...
            cursos.type
        FROM aluno_curso
    LEFT JOIN cursos ON aluno_curso.id_curso = cursos.id
    LEFT JOIN alunos ON aluno_curso.id_aluno = alunos.id
            WHERE alunos.email = '{$email}'
";
...
function generate_image($args = false) {
   ...
   $args['data'] = "Brasília ".strftime('%d de %B de %Y.', strtotime('today'));

    $params = array('name', 'data');//parametro dois envia a data para o template 
