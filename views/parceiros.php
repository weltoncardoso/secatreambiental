<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <link href="https://fonts.googleapis.com/css?family=Poppins:400,800,900" rel="stylesheet">
    <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.2.0/css/all.css" integrity="sha384-hWVjflwFxL6sNzntih27bfxkr27PmbbK/iSvJ+a4+0owXq79v+lsFkW54bOGbiDQ" crossorigin="anonymous">
    <?php define("BASE", "http://localhost/secatre1.0/") ?>
    <link rel="shortcut icon" href="<?php echo BASE; ?>assets/img/thor.ico">
    <link rel="stylesheet" href="<?php echo BASE; ?>assets/fonts/font-awesome/css/all.css" />
    <link rel="stylesheet" href="<?php echo BASE; ?>assets/css/bootstrap.min.css">
    <link rel="stylesheet" href="<?php echo BASE; ?>assets/css/style.css">

    <title>Estratégia Ambiental</title>
  </head>
  <body>

    <?php include('header.php'); ?>

    <div class="tab">
      <div class="container">
        Parceiros
      </div>
    </div>

    <section class="partners py-5 my-5">
      <div class="container">
        <div class="row">
          <div class="col-md-2 col-sm-6 col-6 mt-5">
            <a href=""><img src="<?php echo BASE; ?>assets/img/cocamar.jpg" alt="Cocamar"></a>
          </div>
          <div class="col-md-2 col-sm-6 col-6 mt-5">
            <a href=""><img src="<?php echo BASE; ?>assets/img/sicredi.jpg" alt="Sicredi"></a>
          </div>
          <div class="col-md-2 col-sm-6 col-6 mt-5">
            <a href=""><img src="<?php echo BASE; ?>assets/img/faep.jpg" alt="Faep"></a>
          </div>
          <div class="col-md-2 col-sm-6 col-6 mt-5">
            <a href=""><img src="<?php echo BASE; ?>assets/img/cocamar.jpg" alt="Cocamar"></a>
          </div>
          <div class="col-md-2 col-sm-6 col-6 mt-5">
            <a href=""><img src="<?php echo BASE; ?>assets/img/sicredi.jpg" alt="Sicredi"></a>
          </div>
          <div class="col-md-2 col-sm-6 col-6 mt-5">
            <a href=""><img src="<?php echo BASE; ?>assets/img/faep.jpg" alt="Faep"></a>
          </div>
        </div>
      </div>
    </section>

    <section class="agronomos py-5 my-5">

      <div class="container">

        <div class="row">
          <h4>Agronomos Parceiros</h4>
        </div>

        <div class="row">
          <div class="col-md-3 hidden-xs">
            <div class="list-group">
              <a href="#" class="list-group-item active">Grupo Agronomia</a>
              <a href="#" class="list-group-item">Grupo Agronomia</a>
              <a href="#" class="list-group-item">Grupo Agronomia</a>
              <a href="#" class="list-group-item">Grupo Agronomia</a>
              <a href="#" class="list-group-item">Grupo Agronomia</a>
              <a href="#" class="list-group-item">Grupo Agronomia</a>
              <a href="#" class="list-group-item">Grupo Agronomia</a>
              <a href="#" class="list-group-item">Grupo Agronomia</a>
              <a href="#" class="list-group-item">Grupo Agronomia</a>
            </div>
          </div>

          <div class="col-md-9 diretoria">
            <div class="row">
              <div class="col-sm-12 col-md-4">
                <div class="thumbnail">
                  <img src="img/agronomo.jpg" alt="Nome Agronomo" class="img-fluid">
                  <div class="caption">
                    <p>Empresa Agronomo</p>
                    <h3>Nome Agronomo</h3>
                    <p>email@email.com.br</p>
                  </div>
                </div>
              </div>
              <div class="col-sm-12 col-md-4">
                <div class="thumbnail">
                  <img src="<?php echo BASE; ?>assets/img/agronomo.jpg" alt="Nome Agronomo" class="img-fluid">
                  <div class="caption">
                    <p>Empresa Agronomo</p>
                    <h3>Nome Agronomo</h3>
                    <p>email@email.com.br</p>
                  </div>
                </div>
              </div>
              <div class="col-sm-12 col-md-4">
                <div class="thumbnail">
                  <img src="<?php echo BASE; ?>assets/<?php echo BASE; ?>assets/img/agronomo.jpg" alt="Nome Agronomo" class="img-fluid">
                  <div class="caption">
                    <p>Empresa Agronomo</p>
                    <h3>Nome Agronomo</h3>
                    <p>email@email.com.br</p>
                  </div>
                </div>
              </div>
              <div class="col-sm-12 col-md-4">
                <div class="thumbnail">
                  <img src="<?php echo BASE; ?>assets/img/agronomo.jpg" alt="Nome Agronomo" class="img-fluid">
                  <div class="caption">
                    <p>Empresa Agronomo</p>
                    <h3>Nome Agronomo</h3>
                    <p>email@email.com.br</p>
                  </div>
                </div>
              </div>
              <div class="col-sm-12 col-md-4">
                <div class="thumbnail">
                  <img src="<?php echo BASE; ?>assets/img/agronomo.jpg" alt="Nome Agronomo" class="img-fluid">
                  <div class="caption">
                    <p>Empresa Agronomo</p>
                    <h3>Nome Agronomo</h3>
                    <p>email@email.com.br</p>
                  </div>
                </div>
              </div>
              <div class="col-sm-12 col-md-4">
                <div class="thumbnail">
                  <img src="<?php echo BASE; ?>assets/<?php echo BASE; ?>assets/<?php echo BASE; ?>assets/img/agronomo.jpg" alt="Nome Agronomo" class="img-fluid">
                  <div class="caption">
                    <p>Empresa Agronomo</p>
                    <h3>Nome Agronomo</h3>
                    <p>email@email.com.br</p>
                  </div>
                </div>
              </div>
              <div class="col-sm-12 col-md-4">
                <div class="thumbnail">
                  <img src="<?php echo BASE; ?>assets/<?php echo BASE; ?>assets/img/agronomo.jpg" alt="Nome Agronomo" class="img-fluid">
                  <div class="caption">
                    <p>Empresa Agronomo</p>
                    <h3>Nome Agronomo</h3>
                    <p>email@email.com.br</p>
                  </div>
                </div>
              </div>
              <div class="col-sm-12 col-md-4">
                <div class="thumbnail">
                  <img src="<?php echo BASE; ?>assets/img/agronomo.jpg" alt="Nome Agronomo" class="img-fluid">
                  <div class="caption">
                    <p>Empresa Agronomo</p>
                    <h3>Nome Agronomo</h3>
                    <p>email@email.com.br</p>
                  </div>
                </div>
              </div>
              <div class="col-sm-12 col-md-4">
                <div class="thumbnail">
                  <img src="<?php echo BASE; ?>assets/img/agronomo.jpg" alt="Nome Agronomo" class="img-fluid">
                  <div class="caption">
                    <p>Empresa Agronomo</p>
                    <h3>Nome Agronomo</h3>
                    <p>email@email.com.br</p>
                  </div>
                </div>
              </div>
              <div class="col-sm-12 col-md-4">
                <div class="thumbnail">
                  <img src="<?php echo BASE; ?>assets/img/agronomo.jpg" alt="Nome Agronomo" class="img-fluid">
                  <div class="caption">
                    <p>Empresa Agronomo</p>
                    <h3>Nome Agronomo</h3>
                    <p>email@email.com.br</p>
                  </div>
                </div>
              </div>
              <div class="col-sm-12 col-md-4">
                <div class="thumbnail">
                  <img src="<?php echo BASE; ?>assets/img/agronomo.jpg" alt="Nome Agronomo" class="img-fluid">
                  <div class="caption">
                    <p>Empresa Agronomo</p>
                    <h3>Nome Agronomo</h3>
                    <p>email@email.com.br</p>
                  </div>
                </div>
              </div>
              <div class="col-sm-12 col-md-4">
                <div class="thumbnail">
                  <img src="<?php echo BASE; ?>assets/img/agronomo.jpg" alt="Nome Agronomo" class="img-fluid">
                  <div class="caption">
                    <p>Empresa Agronomo</p>
                    <h3>Nome Agronomo</h3>
                    <p>email@email.com.br</p>
                  </div>
                </div>
              </div>
              <div class="col-sm-12 col-md-4">
                <div class="thumbnail">
                  <img src="<?php echo BASE; ?>assets/img/agronomo.jpg" alt="Nome Agronomo" class="img-fluid">
                  <div class="caption">
                    <p>Empresa Agronomo</p>
                    <h3>Nome Agronomo</h3>
                    <p>email@email.com.br</p>
                  </div>
                </div>
              </div>
              <div class="col-sm-12 col-md-4">
                <div class="thumbnail">
                  <img src="<?php echo BASE; ?>assets/img/agronomo.jpg" alt="Nome Agronomo" class="img-fluid">
                  <div class="caption">
                    <p>Empresa Agronomo</p>
                    <h3>Nome Agronomo</h3>
                    <p>email@email.com.br</p>
                  </div>
                </div>
              </div>
              <div class="col-sm-12 col-md-4">
                <div class="thumbnail">
                  <img src="<?php echo BASE; ?>assets/img/agronomo.jpg" alt="Nome Agronomo" class="img-fluid">
                  <div class="caption">
                    <p>Empresa Agronomo</p>
                    <h3>Nome Agronomo</h3>
                    <p>email@email.com.br</p>
                  </div>
                </div>
              </div>
              <div class="col-sm-12 col-md-4">
                <div class="thumbnail">
                  <img src="img/agronomo.jpg" alt="Nome Agronomo" class="img-fluid">
                  <div class="caption">
                    <p>Empresa Agronomo</p>
                    <h3>Nome Agronomo</h3>
                    <p>email@email.com.br</p>
                  </div>
                </div>
              </div>
              <div class="col-sm-12 col-md-4">
                <div class="thumbnail">
                  <img src="<?php echo BASE; ?>assets/img/agronomo.jpg" alt="Nome Agronomo" class="img-fluid">
                  <div class="caption">
                    <p>Empresa Agronomo</p>
                    <h3>Nome Agronomo</h3>
                    <p>email@email.com.br</p>
                  </div>
                </div>
              </div>
              <div class="col-sm-12 col-md-4">
                <div class="thumbnail">
                  <img src="<?php echo BASE; ?>assets/img/agronomo.jpg" alt="Nome Agronomo" class="img-fluid">
                  <div class="caption">
                    <p>Empresa Agronomo</p>
                    <h3>Nome Agronomo</h3>
                    <p>email@email.com.br</p>
                  </div>
                </div>
              </div>
              <div class="col-sm-12 col-md-4">
                <div class="thumbnail">
                  <img src="<?php echo BASE; ?>assets/img/agronomo.jpg" alt="Nome Agronomo" class="img-fluid">
                  <div class="caption">
                    <p>Empresa Agronomo</p>
                    <h3>Nome Agronomo</h3>
                    <p>email@email.com.br</p>
                  </div>
                </div>
              </div>
              <div class="col-sm-12 col-md-4">
                <div class="thumbnail">
                  <img src="<?php echo BASE; ?>assets/img/agronomo.jpg" alt="Nome Agronomo" class="img-fluid">
                  <div class="caption">
                    <p>Empresa Agronomo</p>
                    <h3>Nome Agronomo</h3>
                    <p>email@email.com.br</p>
                  </div>
                </div>
              </div>
            </div>
          </div>

        </div>
      </div>



    </section>

    <?php include('footer.php'); ?>

  </body>
</html>
