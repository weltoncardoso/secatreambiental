...
<table width="65%" style="margin: 0 15% 0 15%" >
	...
	<?php foreach($resolucoesfederais as $not):?>
		<tr>
  ...
  border-bottom: 2px solid #fff; font-family: sans-serif; line-height: 1.5; font-size: 15px" align="center">
        <a title="Click para visualizar o documento!" target="_blank" href="<?php echo BASE; ?>resolucoesFederais/view/<?php echo $not['id']; ?>">
          <?php echo ($not['nome']); ?></a>
        </td>
        <td style="background-color: #dee2e6;
...
<td colspan="2" style="margin-left:10px">
 <div class="pagination_leis">
        <div class="pag_item" ><a href="<?php echo BASE; ?>resolucoesFederais?p=<?php echo $p-1; ?>" aria-label="Previous">
        <span aria-hidden="true">&laquo;</span></a></div>
        <?php
        $conta = ceil($total_resolucoes_federais = 3);
        for($q=1;$q <= $conta;$q++): ?>
        <div class="pag_item <?php echo($q==$p)?'pag_ativo':''; ?>"><a href="<?php echo BASE; ?>resolucoesFederais?p=<?php echo $q; ?>"><?php echo $q; ?></a></div>
        <?php endfor; ?>
        <?php if($p>$conta): ?>
          <div style="color: #FFFFFF" class="pag_item <?php echo $p?'pag_ativo':''; ?>"><?php echo $p; ?></div>
          <?php endif; ?>
        <div class="pag_item" ><a href="<?php echo BASE; ?>resolucoesFederais?p=<?php echo $p+1; ?>" aria-label="Next">
